...
import { connectWallet, createCandidate } from '../Api/api'
...
// Submit Function
  ...
  const submit = async() => {
    if (details.name === '' || details.age === '' || details.address === ''){
      ...
      if (checkaAddess) {
        const data = await createCandidate(details.address,details.age,details.name);
        if (data === "Candidate created sucessfully"){
          alert(data);
          let path = `/`; 
          navigate(path);
        }
...
const isValidAddress = (adr) => {
  ...
  return (
    <div>
      ...
      <h1 style={{color: "white", fontWeight: "600"}}>Create Candidate</h1>
      </div>
    ...
    <div className='container'>
      <div className='main_container'>
        ...
        <Input place="Name" func={(e) => {setDetails({...details, name: e.target.value})}}/>
        <Input place="Age" func={(e) => {setDetails({...details, age: e.target.value})}}/>
        <Input place="Address" func={(e) => {setDetails({...details, address: e.target.value})}}/>
        <Button func={submit}/>
      </div>      
    </div>
    </div>
